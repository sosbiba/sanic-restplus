...
class APITest(object):
    ...
    async def test_api_delayed_initialization(self, app, client):
    ...
    def test_api_prefix(self, app):
        api = sanic_restplus.Api(app, prefix='/foo')
    ...
    def test_add_two_conflicting_resources_on_same_endpoint(self, app):
        ...
        class Foo2(sanic_restplus.Resource):
            async def get(self, request):
                return 'foo2'
    ...
    async def test_add_the_same_resource_on_same_endpoint(self, app):
        api = sanic_restplus.Api(app)
        ...
        with app.test_client() as client:
            foo1 = await client.get('/foo')
            assert foo1.data == b'"foo1"\n'
            foo2 = await client.get('/foo/toto')
    ...
    async def test_add_resource_forward_resource_class_parameters(self, app, client):
        api = sanic_restplus.Api(app)
    ...
    def test_resource(self, app, mocker):
        resource = sanic_restplus.Resource()
    ...
    async def test_read_json_settings_from_config(self, app, client):
        ...
        data = await client.get('/foo').data

        expected = b'{\n  "baz": "qux", \n  "foo": "bar"\n}\n'

        assert data == expected

    async def test_use_custom_jsonencoder(self, app, client):
        ...
        class Cabbage(sanic_restplus.Resource):
            async def get(self, request):
                return {'frob': object()}

        api.add_resource(Cabbage, '/cabbage')

        data = await client.get('/cabbage').data

        expected = b'{"frob": "cabbage"}\n'
        assert data == expected

    async def test_json_with_no_settings(self, api, client):
        class Foo(sanic_restplus.Resource):
            async def get(self, request):
                return {'foo': 'bar'}
    ...
    async def test_redirect(self, api, client):
        class FooResource(sanic_restplus.Resource):
            async def get(self, request):
                return redirect('/')
